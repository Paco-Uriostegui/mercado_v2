...
class NewauthGateBloc extends Bloc<NewAuthGateEvent, NewAuthGateState> implements NewIAuthGateStream {
  final StreamController<NewAuthenticationStatus> _controller = StreamController<NewAuthenticationStatus>();
  late StreamSubscription<AuthUserStatus> _subscription;
  final AuthenticationUseCase _usecase;


  NewauthGateBloc(AuthenticationUseCase usecase) : _usecase = usecase, super(NewAuthGateState.loading()) {
    on<InitStream>(_onInitStream);
  }

  @override
  Stream<NewAuthenticationStatus> get authenticationGateStream => _controller.stream;

  Future<void> _onInitStream(NewAuthGateEvent event, Emitter<NewAuthGateState> emit) async {
